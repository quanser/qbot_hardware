...
int main(int argc, char ** argv)
{
    ...
    // The Leishen M10P physically caps at ~12 Hz at the device level; that hardware
    // limit is what actually gates how often new data is ready. `poll_rate` below is
    // just how often we *check* for that new data -- it should be well above the
    // device's native rate, so we notice a new scan almost as soon as it's ready
    // instead of adding extra wait time on top of the sensor's own cadence.
    // `nominal_publish_rate` is kept only for the initial (previousScanTime == 0)
    // time_increment estimate before we have a real measured scan_time to use.
    double poll_rate = 100;             // Hz. Poll well above the ~12 Hz device cap.
    double nominal_publish_rate = 12;   // Hz. Best-effort estimate of device's native rate.
...
would not be sent on the /scan message.";
    ...
    if (result >= 0)
    {
        ...
        rclcpp::WallRate loop_rate((double)poll_rate);
        node_running = true;
        while (rclcpp::ok())
        {
            /*
            ...
            */
            ...
            if (result >= 0 && ((t_uint32)result >= (num_scan_points - ((1 - scan_mode) * num_allowable_invalid_data))))
            {
                ...
                // Use the actually measured interval between scans rather than an
                // assumed fixed rate, since the device's real cadence can vary. Fall
                // back to a nominal estimate only for the very first scan, when there
                // is no previous timestamp yet to measure from.
                msgs.scan_time = has_previous_scan
                    ? (currentScanTime.seconds() - previousScanTime.seconds())
                    : (1.0 / nominal_publish_rate);
                msgs.time_increment = msgs.scan_time / result;
