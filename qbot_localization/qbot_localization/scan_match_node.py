...
class ScanMatchNode(Node):
    def __init__(self):
        ...
        self.init_search = np.array(self.init_pose, dtype=np.float32)
    ...
    def calibrate(self, msg: LaserScan):
        ...
        if num_points>300 and angle_range > 355/360*np.pi \
            ...
            self.get_logger().info('Calibration complete. Reference scan saved at ' + self.ref_scan_path)
    ...
    def scan_callback(self, msg: LaserScan):
        ...
        if self.reference_scan is None:
            ...
            processed_x = estimated_pose[1]  - 0.22*np.cos(estimated_pose[2]) # lidar to center of qbot
            processed_y = -estimated_pose[0] - 0.22*np.sin(estimated_pose[2]) 
    ...
    def handle_reset(self, request, response):
        self.start_time = self.get_clock().now() - Duration(seconds=3)  # minimum time to wait for calibration
        self.init_search[:] = self.init_pose                            # such that reference scan saves immediately after reset
        self.reference_scan = None
        self.calibrate_flag = True
        self.get_logger().info('Localization search state reset.')
        response.success = True
        response.message = 'Localization search state reset.'
        return response

    def match_scan(self, scan: LaserScan):
        num_points,curr_angle,curr_range = self._preprocess(scan)
    ...
    def terminate(self):
        ...
        # self.timing_log_file.flush()
        # self.timing_log_file.close()
